...
import { AddDiv, Btn, Input ,DivImg} from './StyledComponents';


export default function BankPayStyled() {
  ...
  {/* Card Name */}
            <Input placeholder='Card Name' readOnly value={index !== -1? list[index].name : ''}></Input>
  {/* Card Number */}
        <Input placeholder='Card Number' value={index !== -1? list[index].num : ''}></Input>
  {/* Destination Card Number */}
          <DivImg>
            <img  alt="logo2" src={logo2} style={{display: logo2? 'flex': 'none'}}/>
            <Input placeholder='Destination Card Number' value={cardNumber3} onChange={(e:React.ChangeEvent<HTMLInputElement>)=> setCardNumber3(regex2.test(e.target.value)? cardNumber3:e.target.value.length>16?cardNumber3: e.target.value )} onKeyDown={(e:React.KeyboardEvent<HTMLInputElement>)=>e.which === 32? e.preventDefault():''}></Input>
          </DivImg> 
  {/* Password */}
            <Input placeholder='Password' value={pass} onChange={(e:React.ChangeEvent<HTMLInputElement>)=>setPass(e.target.value)} type='password'></Input>
  {/* Amount */}
        <Input placeholder='Amount' value={asset2===0?'':asset2} onChange={(e:React.ChangeEvent<HTMLInputElement>)=> setAsset2(regex2.test((e.target.value))? asset2:e.target.value.length>=10? asset2: +(e.target.value))}></Input>
  {/* Btn */}
        <Btn onClick={()=>{setL(true);pay();{<Alert severity="error">This is an error alert — check it out!</Alert>}}}><SendSharpIcon fontSize={'large'}/>Pay</Btn>
    </AddDiv>
  )
}
